...
def main():
    ...
    cropped_img, bbox = image.crop_image_randomly(img, (42, 42))

    tps_random_rate = 0
    #moving_vectors = (np.random.rand(9, 2) - 0.5) * 2 * tps_random_rate
    moving_vectors = np.zeros([9, 2])
    moving_vectors[0, 0] += 0.1
    print("moving_vectors : ", moving_vectors)

    pad_ratio = None

    warped_image1, mv = image.synthesize_image(
        img, bbox=bbox, pad_ratio=pad_ratio, moving_vectors=moving_vectors, tps_random_rate=0)
    print("returned moving_vectors : ", mv)
    warped_image2, mv = image.synthesize_image(
        cropped_img, bbox=None, pad_ratio=pad_ratio, moving_vectors=moving_vectors, tps_random_rate=0)
    print("returned moving_vectors : ", mv)
